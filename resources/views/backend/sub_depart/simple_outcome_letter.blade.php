                    ...
                    <table class="table table-bordered" id="paginate" width="100%" cellspacing="0">
                      ...
                      <tbody>
                         @if(count($outcome_letters) > 0)
                            ...
                            <td>{{$index+1}}</td>
                                <td>{{$data['date']}}</td>
                                <td><a href="{{url('sub_depart/single/letter/'.$data['id'].'/'.$data['to_sub_depart_name'])}}">{{$data['letter_no']}}</a></td>
                                <td>{{$data['title']}}</td>
...
@section('js')
    <script>
        ...
        $(document).on('click','.all_outcome_letter',function(){ // all outcome letters
            form.reset();
            table.clear().draw();
            let url="{{url('sub_depart/simple/all/outcome-letter')}}";
            $.ajax({
                url : url,
                type : "get",
                dataType : "json"
                ...
                response.map((data, index) => {
                    let no = index + 1;
                    let date=data['date'];
                    let url="{{url('sub_depart/single/letter')}}/"+data['id']+"/"+data['to_sub_depart_name'];
                    ...
                    table.row.add([no,date,letter_no,title,detail,created,to,level,download]).draw();
                });
            }).fail(function(error){
                console.log(error);
            });
        ...
        $(document).on('click','.daily_outcome_letter',function(){ // daily outcome data
            form.reset();
            table.clear().draw();
            let url="{{url('sub_depart/simple/outcome-letter')}}";
            $.ajax({
                url : url,
                type : "get",
                dataType : "json"
        ...
        $(document).on('submit','#search',function(e){ // search outcome letter
          ...
          let url = "{{url('sub_depart/search/outcome_letter')}}";
            $.ajax({
              url: url,
              type: "post",
              data: form_data,
